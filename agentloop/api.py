...
ROLE_ORDER = ["analyst", "architect", "implementer", "tester", "reviewer", "integrator"]
...
def available_actions(state: dict[str, Any], lock_reason: str | None = None) -> dict[str, Any]:
    ...
    cancel_enabled = status not in {"CREATED", "DONE", "CANCELLED"}
    return {
        ...
        "delete": action(True),
        "config": action(True),
        "analysis_review": action(status == "WAITING_FOR_ANALYSIS_REVIEW", None if status == "WAITING_FOR_ANALYSIS_REVIEW" else f"status is {status}"),
    }


def build_analysis_review(state: dict[str, Any]) -> dict[str, Any] | None:
    if state.get("status") != "WAITING_FOR_ANALYSIS_REVIEW":
        return None
    questions = state.get("analysis_questions") if isinstance(state.get("analysis_questions"), list) else []
    return {
        "required": True,
...
def create_task(root: Path, payload: dict[str, Any]) -> dict[str, Any]:
    ...
    state = start_task(
        ...
        require_analysis_review=True,
        run_analyst=False,
    )
    return build_task_detail(root, str(state["task_id"]))


def submit_analysis_review_api(root: Path, task_id: str, payload: dict[str, Any]) -> dict[str, Any]:
    ...
    with task_lock(root, task_id, blocking=False):
        state = load_task_state(root, task_id)
        if state.get("status") != "WAITING_FOR_ANALYSIS_REVIEW":
            raise WorkspaceError(f"Cannot submit analysis review while status is {state.get('status')}.")
        questions = state.get("analysis_questions") if isinstance(state.get("analysis_questions"), list) else []
...
def patch_task_config(root: Path, task_id: str, payload: dict[str, Any]) -> dict[str, Any]:
    allowed = {"max_iterations", "test_commands"}
